...
| **Turno real (`DraftState.turn`)** | Rediseño UX de `/draft` (`/grill-me`, 2026-08-23, TSK-071 a TSK-074): lado + acción esperada + reloj, calculado puro contra una tabla de turnos curada (`draft-format-turns.json`, mismo patrón que `hero-positions.json`) — nunca lógica adivinada del reductor. Cubre **ambos** formatos (Captain's Mode y All Pick), decisión explícita del usuario. |
| **Reserva bancable (banked time)** | Sistema de tiempo real de Captain's Mode competitivo: tiempo base por turno + un banco de reserva por equipo que se gasta cuando el base se acaba. Decisión explícita del usuario: replicarlo tal cual, no un contador fijo simplificado (TSK-071/072). |
| **`wrong_turn`** | `RejectionReason` nuevo (TSK-072) — un pick/ban fuera del turno esperado se rechaza igual que `wrong_phase`/`stale_seq`. `pick_reverted` queda exento, mismo criterio que ya tiene con `stale_seq`. Decisión explícita del usuario: validación dura, no solo informativa. |
