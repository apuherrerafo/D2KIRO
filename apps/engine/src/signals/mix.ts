...
function mixScore(signals: SignalContribution[]): number {
  ...
  const totalWeight = withData.reduce((sum, s) => sum + SCORING_WEIGHTS_V1[s.signal], 0);
  return withData.reduce((sum, s) => {
    const share = SCORING_WEIGHTS_V1[s.signal] / totalWeight; // redistribución proporcional
...
function buildReason(signals: SignalContribution[]): string {
  const informative = signals
    .filter((s) => s.raw !== null)
    .sort((a, b) => SCORING_WEIGHTS_V1[b.signal] - SCORING_WEIGHTS_V1[a.signal])
    .slice(0, 2)
    .map((s) => s.explanation);
